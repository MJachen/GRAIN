...
| Eq. (5)–(8) | Separate patient embeddings and relation enhancement | `AdaptivePatientGraph.forward` | target `[B,P]`, anchors `[A,P]` → relation embeddings | Gradient tested; activation/gamma ASSUMED and alpha VERIFIED |
...
| Table II/VI/VII | Center A/B, mixed A+B, external Center C | `configs/formal_*.json` | bound data/splits → future experiment | A/B configs frozen and loadable; Mixed mask and Mixed-to-C feature space BLOCKED |
...
## Frozen architecture provenance

Formal configs now resolve every construction/training field. Four parameters
remain explicitly `ASSUMED` rather than falsely marked verified: patient width,
one-token within-patient attention, relation activation, and relation gamma.
See `configs/architecture_grain_official.json` and
`docs/architecture_provenance.md` for the evidence table. This does not permit
outer-test-based architecture selection.
